...
class DRDataset:
    def __init__(
        self, dataset_name="Fdataset", drug_neighbor_num=15, disease_neighbor_num=15
    ):
        ...
        print(
            f"dataset:{dataset_name}, drug:{self.drug_num}, disease:{self.disease_num}, pos weight:{self.pos_weight}"
        )
...
class Dataset:
    def __init__(self, dataset, mask, fill_unkown=True, stage="train", **kwargs):
        ...
        if not fill_unkown:
            ...
            self.label = (
                torch.from_numpy(dataset.interactions[mask]).float().contiguous()
            )
            ...
            row_idx, col_idx = torch.meshgrid(
                torch.arange(mask.shape[0]), torch.arange(mask.shape[1])
            )
            self.interaction_edge = torch.stack(
                [row_idx.reshape(-1), col_idx.reshape(-1)]
            )
    ...
    def get_v_edge(self, union_graph=False):
        ...
        if union_graph:
            edge_index = edge_index + torch.tensor(
                np.array([[self.size_u], [self.size_u]])
            )
            size = (self.size_u + self.size_v,) * 2
        return edge_index, value, size
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(dataset) < batch_size:
            logging.info(
                f"dataset size:{len(dataset)}, batch_size:{batch_size} is invalid!"
            )
        ...
        super(GraphDataIterator, self).__init__(
            dataset=dataset,
            batch_size=None,
...
class CVDataset(pl.LightningDataModule):
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument(
            "--split_mode", default="global", choices=["global", "local"]
        )
        ...
        parser.add_argument(
            "--global_test_all_zero",
            default=False,
            action="store_true",
            help="全局模式每折测试集是否测试所有未验证关联，默认：不测试",
        )
        parser.add_argument(
            "--train_fill_unknown",
            default=True,
            action="store_true",
            help="训练集中是否将测试集关联填0还是丢弃，默认：丢弃",
        )
        parser.add_argument(
            "--dataset_type",
            default=None,
            choices=["FullGraphDataset", "PairGraphDataset"],
        )
        parser.add_argument("--seed", default=666, type=int)
        return parent_parser

    def fold_mask_iterator(
        self,
        interactions,
        mode="global",
        n_splits=10,
        ...
        if mode == "global":
            if n_splits == 1:
                ...
                for (train_pos_idx, test_pos_idx), (train_neg_idx, test_neg_idx) in zip(
                    kfold.split(pos_row), kfold.split(neg_row)
                ):
                    ...
                    train_pos_edge = np.stack(
                        [pos_row[train_pos_idx], pos_col[train_pos_idx]]
                    )
                    train_neg_edge = np.stack(
                        [neg_row[train_neg_idx], neg_col[train_neg_idx]]
                    )
                    test_pos_edge = np.stack(
                        [pos_row[test_pos_idx], pos_col[test_pos_idx]]
                    )
                    test_neg_edge = np.stack(
                        [neg_row[test_neg_idx], neg_col[test_neg_idx]]
                    )
                    train_edge = np.concatenate(
                        [train_pos_edge, train_neg_edge], axis=1
                    )
            ...
            if row_idx is not None:
                row_idxs = (
                    list(range(interactions.shape[0])) if n_splits == -1 else [row_idx]
                )
                for idx in row_idxs:
                    yield self.get_fold_local_mask(interactions, row_idx=idx)
            elif col_idx is not None:
                col_idxs = (
                    list(range(interactions.shape[1])) if n_splits == -1 else [col_idx]
                )
                for idx in col_idxs:
    ...
    def get_fold_local_mask(self, interactions, row_idx=None, col_idx=None):
        ...
        if row_idx is not None:
            ...
            test_mask[
                np.ones(interactions.shape[1], dtype="int") * row_idx,
            ...
            test_mask[
                ...
                np.ones(interactions.shape[0], dtype="int") * col_idx,
            ] = True
    ...
    def prepare_data(self):
        ...
        if len(glob.glob(os.path.join(save_dir, "split_*.mat"))) != len(self):
            for i, (train_mask, test_mask) in enumerate(
                ...
                scio.savemat(
                    os.path.join(save_dir, f"split_{i}.mat"),
    ...
    def train_dataloader(self):
        return GraphDataIterator(
            self.dataset,
            self.train_mask,
            fill_unkown=self.train_fill_unknown,
            stage="train",
...
def load_DRIMC(root_dir="dataset/LRSSL", name="c", reduce=True):
    ...
    """
    drug_chemical = pd.read_csv(
        os.path.join(root_dir, f"{name}_simmat_dc_chemical.txt"), sep="\t", index_col=0
    )
    drug_domain = pd.read_csv(
        os.path.join(root_dir, f"{name}_simmat_dc_domain.txt"), sep="\t", index_col=0
    )
    drug_go = pd.read_csv(
        os.path.join(root_dir, f"{name}_simmat_dc_go.txt"), sep="\t", index_col=0
    )
    disease_sim = pd.read_csv(
        os.path.join(root_dir, f"{name}_simmat_dg.txt"), sep="\t", index_col=0
    )
    if reduce:
    ...
    drug_disease = pd.read_csv(
        os.path.join(root_dir, f"{name}_admat_dgc.txt"), sep="\t", index_col=0
    ).T
    if name == "lrssl":
    ...
    return {"drug": rr, "disease": dd, "Wrname": rname, "Wdname": dname, "didr": rd.T}


def load_HDVD(root_dir="dataset/hdvd"):
    """drug:219, virus:34, association: 455"""
    dd = pd.read_csv(os.path.join(root_dir, "virussim.csv"), index_col=0).to_numpy(
        np.float32
    )
    rd = pd.read_csv(os.path.join(root_dir, "virusdrug.csv"), index_col=0)
    rr = pd.read_csv(os.path.join(root_dir, "drugsim.csv"), index_col=0).to_numpy(
        np.float32
    )
